...
if not st.session_state.ready_to_start:
    ...
    st.write("You are about to begin the BIO 102 Exam. Once you start, the 5-minute timer will run.")
    ...
    if col1.button("Start Exam"):
        ...
        st.rerun()
    if col2.button("Quit"):
        st.warning("Exam cancelled. You may close this tab.")
        st.stop()
    st.stop()
...
if not st.session_state.submitted:
    ...
    if st.session_state.confirm_submit:
        st.warning("⚠️ Are you sure? You cannot retake the exam.")
        c1, c2 = st.columns(2)
    ...
    time.sleep(1) # Ensures the timer updates visually
    st.rerun()
